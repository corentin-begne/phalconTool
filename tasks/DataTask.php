    ...
    public function importAction() { 
        ...
        foreach(glob($this->config->application->dumpDir.'*.csv') as $file){
            ...
            if(defined('TRUNCATE')){
                ...
                exec('export MYSQL_PWD='.$this->config[ENV]->database->password.';mysql -u '.$this->config[ENV]->database->username.' --database '.$this->config[ENV]->database->dbname.' -h '.$this->config[ENV]->database->host.' --port 3306 -ss -e "'.$query.'"  > /dev/null 2>>'.$log);
                $this->checkLog($log);
            ...
            exec('export MYSQL_PWD='.$this->config[ENV]->database->password.';mysql --local-infile --show-warnings --verbose --default-character-set=utf8 -u '.$this->config[ENV]->database->username.' --database '.$this->config[ENV]->database->dbname.' -h '.$this->config[ENV]->database->host.' --port 3306 -e "'.$query.'" >> /dev/null 2>>'.$log);
            $this->checkLog($log);
            Cli::success('Success', true);
        }
    }
